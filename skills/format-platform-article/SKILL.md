...
- Zsxq: use `platforms/zsxq.html`. It is body-only and intentionally avoids the WeChat magazine wrapper/background. It uses paste-stable native tags, modest inline margins/line-height, `max-width:100%;height:auto` images, and the text-safe `▍` marker for authored high-value paragraphs. Do not use empty `<p><br></p>` spacer paragraphs to create rhythm; they make long articles look bloated in preview.
