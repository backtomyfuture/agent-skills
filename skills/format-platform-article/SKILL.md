...
description: Convert a local Markdown article and media folder into a WeChat-first multi-platform publish package. Use this skill after notion-to-md exports Markdown/media and before publishing to WeChat Official Account, Zhihu, Toutiao, Zsxq, SMZDM, Xueqiu (雪球), Baijiahao (百家号), Juejin (掘金), Xiaohongshu (小红书), or similar rich-text/Markdown article platforms. Generates paste-ready HTML (WeChat/Toutiao/SMZDM/Xueqiu/Baijiahao), import-ready Markdown (Zhihu via md2zhihu, Zsxq, Juejin), a Xiaohongshu plain-text note draft, local assets, an image fallback manifest, and report.json. You must use this skill whenever the user wants to publish Markdown articles to Chinese social/tech media platforms, format posts for WeChat/Zhihu/Toutiao/Zsxq/SMZDM/雪球/百家号/掘金/小红书, or build a publish package from a Markdown file, even if they don't explicitly ask for a "publish package".
---
...
│   ├── smzdm.html
│   ├── xueqiu.html            # 雪球 long-post (native rich-text HTML)
│   ├── baijiahao.html         # 百家号 (native rich-text HTML)
│   ├── juejin.md              # 掘金 Vditor Markdown (images as [[IMG_N]] placeholders)
│   └── xiaohongshu.md         # 小红书 plain-text note draft (no Markdown; ~1000-char cap)
├── assets/
└── report.json
```
...
- Lift Markdown tables out of blockquotes before rendering. Notion exports a callout that contains a table as a quote block whose every row is prefixed with `> ` — no platform treats a quoted table as a table, so the rows flatten into one paragraph of literal `|` characters on every channel. The build keeps the callout label line as a quote/callout and promotes the `> | ... |` rows to a top-level table, restoring the normal table pipeline (PNG conversion for WeChat/HTML platforms, native tables for Zsxq/Juejin, md2zhihu for Zhihu).
...
- The same normalization pass cleans the stray spaces Notion leaves around bold spans: `约  **9 亿** ，已有` becomes `约 **9 亿**，已有`. Interior runs of spaces collapse to one, spaces hugging full-width punctuation are dropped, and a space between two CJK characters (seen through `**` markers) is removed — while the deliberate pangu spacing between CJK and Latin/digits (`长 PDF`) is kept. Without this, every platform output shows double gaps and a floating space before `，`.
...
- Render Markdown dividers (`---`, `***`, `___`) as real-character separators (the editorial three-dot `● ● ●` in muted gold for WeChat) rather than empty CSS-only rules. WeChat often strips blank `div`/`hr` separators during copy/paste, so a visible character-based mark is required. Avoid templated single em-dashes — they read as accidental punctuation. The native flavors (Toutiao/SMZDM/Baijiahao/Xueqiu) emit the same `● ● ●` character divider — a pasted `<hr>` is unreliable in those editors and a dropped divider silently runs the author's sections together. Markdown outputs (Zsxq/Juejin) keep the raw `---` since their editors render a native rule.
...
- Toutiao: use `platforms/toutiao.html`. The Toutiao editor aggressively strips pasted decorative CSS (background colours, `<section>` wrappers, styled-`<span>` emphasis), so this output deliberately drops the WeChat magazine layout and uses conservative **native** tags instead — `<h2>`/`<h3>`, `<strong>`/`<em>`, `<ul>`/`<ol>`, native `<blockquote>` for callouts and quotes (a bold "emoji 标签：" lead, no coloured card), `<pre><code>`, and Base64 `<img>`. The key reason for native `<strong>` over a styled span: if the editor strips the style, a `<span style="font-weight">` loses its bold entirely, whereas `<strong>` keeps it. Live-tested in the Toutiao editor: Base64 `<img>` is **auto-uploaded to Toutiao's CDN on paste** (so embedded images work — no manual upload needed), and `<h2>`/`<strong>`/`<blockquote>` are remapped to Toutiao's native heading/bold/quote. The one platform limit (not a formatting bug): **external links in the body are stripped** — like WeChat, the link text stays but the `href` is removed. Keep links as text and route any essential one through Toutiao's own mechanisms.
- Zsxq: prefer `platforms/zsxq.md`. Zsxq's long-article editor has two modes (toolbar toggle): rich-text (Quill) and **Markdown (Milkdown/ProseMirror)**. In Markdown mode, pasted Markdown is parsed into **native** nodes — `>` becomes Zsxq's own blue quote card, `##` a native heading, `**` bold, `- ` a native list, and `| |` a native table. This was verified live in the editor. So `zsxq.md` keeps callouts and quotes as real `>` blockquotes (callouts as `> 💡 **标签** 正文`), native `| |` tables (not rasterized), and `##/###` headings; you paste it in Markdown mode (or via the `publish-zsxq-article` skill). **Images become `[[IMG_N]]` text placeholders annotated with the asset path** — e.g. `[[IMG_1]] 🖼 在此插入图片：assets/diagrams/mermaid_xxx.png` — not `![](localpath)` refs. This is deliberate: a Markdown text paste cannot load local-path images, and inlined data: URIs paste into the draft but Zsxq rejects them at save/publish (verified). So images must be added as **binary** at their spot — paste it as text first, then at each `[[IMG_N]]` marker copy the named PNG and paste it (Zsxq uploads it to its CDN), or let `publish-zsxq-article` binary-paste them automatically. Remote (`http(s)`) images stay as normal `![](url)` refs since the editor loads those. The HTML fallback `platforms/zsxq.html` exists only for rich-text-mode pasting — note its `<blockquote>` is flattened to a plain paragraph there (no native quote), which is why Markdown mode is preferred.
- SMZDM: use `platforms/smzdm.html`. It shares the same conservative native renderer as Toutiao (native `<h2>`/`<strong>`/`<blockquote>`/`<pre>`, Base64 `<img>`, no magazine cards). Treated the same as Toutiao (not separately live-tested), so expect Base64 images to upload and external links to be limited. Note SMZDM's content rules: articles should be a 消费/购物 ("worth buying") topic, ≥800 字, ≥5 images, ideally with product links — a pure analysis/opinion piece is a poor fit regardless of formatting. Convert product URLs to platform cards manually inside the editor.
- Xueqiu (雪球): use `platforms/xueqiu.html`. 雪球 is an investment community, so finance/investing pieces fit best. Its 创作者中心「发布长文」editor (mp.xueqiu.com/writeV2) is ProseMirror; **live-tested** with this output: headings map to 雪球's heading style, **bold** is kept, **Base64 `<img>` is auto-uploaded to 雪球's CDN** (no manual upload), and — unlike WeChat/Toutiao — **external links are kept and clickable**. That last point makes 雪球 unusually good for your funnel: you can put a 公众号/知识星球 link directly in the body. Because 雪球's editor strips `<blockquote>`/`<ul>`/`<hr>`, `xueqiu.html` is rendered with a dedicated **雪球 flavor** that uses only the elements 雪球 keeps: callouts become bold-label paragraphs (`💡 一句话先说结论：…`), list items get a real `•` bullet, and section breaks use a centered `● ● ●` text divider (a plain `<hr>` would vanish, running sections together). Headings, bold, italic, links and images are kept as-is. Paste `xueqiu.html` directly. Best-practice extras to add by hand in 雪球: turn mentioned tickers into `$股票$` cashtags and add a `#话题#` for discovery, and — since links are clickable — drop your 公众号/知识星球 link in the body for conversion.
- Baijiahao (百家号): use `platforms/baijiahao.html`. Baidu's 头条号-equivalent; main value is passive 引流 via Baidu search SEO. Same native renderer as Toutiao. Not separately live-tested; expect external links to be limited.
- Juejin (掘金): use `platforms/juejin.md`. 掘金's editor is a Vditor **Markdown** editor, so it takes full Markdown natively — `##` headings, `**` bold, `| |` tables, ` ``` ` code blocks. Best for developer-facing pieces (引流 technical readers). Images are `[[IMG_N]]` placeholders annotated with the asset path (same reasoning as Zsxq): paste the Markdown, then drag/paste each image at its marker so 掘金 uploads it to its image bed. Remote `![](url)` images are kept as-is.
- Xiaohongshu (小红书): use `platforms/xiaohongshu.md` as a **plain-text draft**. 小红书 notes are plain text + emoji + blank-line paragraphs (no Markdown), body capped around **1000 字**, ≤9 images (3:4) plus a separate cover. This output strips all Markdown to plain text (keeps emoji), flattens tables to ` ｜ `-joined lines, and marks images as `[[IMG_N]]`; the first line is a length hint. Because the cap is ~1000 字, a long article is almost always over — treat this as a starting draft to trim into key points or split across several notes. For actual publishing use the `publish-xiaohongshu-article` skill (it handles 小红书's note flow and image upload). Title and first image (封面) matter most and are chosen separately.
...
- `--zhihu-asset-repo <git-url>`: Git repo md2zhihu pushes Zhihu images to and rewrites as raw HTTPS URLs, e.g. `https://github.com/backtomyfuture/images.git` or `git@github.com:backtomyfuture/images.git`. Resolution order: `$ZHIHU_ASSET_REPO` → `$MD2ZHIHU_ASSET_REPO` → the built-in `DEFAULT_ZHIHU_ASSET_REPO` (`https://github.com/backtomyfuture/images.git`). So **Zhihu images are hosted by default** — every build produces an import-ready `zhihu.md` with working HTTPS images, no env var needed. Pass `--zhihu-asset-repo ""` to force the local `../assets/` fallback.
