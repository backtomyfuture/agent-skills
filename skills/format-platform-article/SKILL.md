...
- Zsxq: use `platforms/zsxq.html`. Its editor strips cards/backgrounds and the audience is paid subscribers who value clarity over decoration, so this output is deliberately plain: paste-stable native tags, modest margins/line-height, `max-width:100%;height:auto` images. Callouts render as a **bold label on its own line** (e.g. `💡 一句话先说结论`) with the body underneath — a clean "小标题 + 正文" block that is easy to scan, not an inline `▍label：body` run-on. The `▍` marker is reserved for genuine `>` quotes (it reads as a quote bar there). Do not promote paragraphs to quotes by keyword, do not re-chunk authored paragraphs, and do not use empty `<p><br></p>` spacers.
