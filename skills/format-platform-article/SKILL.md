...
- Xueqiu (雪球): use `platforms/xueqiu.html`. 雪球 is an investment community, so finance/investing pieces fit best (strong 引流 to a paid group). It shares Toutiao's native rich-text renderer (native `<h2>`/`<strong>`/`<blockquote>`, Base64 `<img>`). Not separately live-tested, so on first publish confirm whether Base64 images upload and external links survive; tune later (e.g. switch to `[[IMG_N]]` placeholders) if needed.
