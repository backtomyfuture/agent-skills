...
Usage:
    ...
    cat notion_raw.md | python3 notion_ingest.py --output-dir /tmp/zsxq-notion

    # Read from a file
    python3 notion_ingest.py --input notion_raw.md --output-dir /tmp/zsxq-notion
...
def download(url: str) -> tuple[bytes, str | None]:
    ...
    req = urllib.request.Request(url, headers={'User-Agent': 'zsxq-skill/1.0'})
    with urllib.request.urlopen(req, timeout=30) as resp:
        return resp.read(), resp.headers.get('Content-Type')


def main():
    parser = argparse.ArgumentParser(description='Ingest Notion Markdown + images for Zsxq')
