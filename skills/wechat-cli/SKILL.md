...
| 秒提/实时看最新多段话 | `latest` | 极速秒提（<0.1s），完整呈现连续多段话，支持 -f 实时流式监听 |
...
| `latest` | 秒提最新消息 | “刚刚谁发了什么”、“秒提最新消息”、“连续多段话” |
...
- “秒提刚才来的最新消息” / “对方打过来几段话帮我看一下” -> `latest`
- “实时监听新消息” / “有消息立刻通知我” -> `latest -f`
...
深入具体消息明细表（`Msg_{hash}`），以极速（<0.1s）提取最新活跃会话或指定聊天对象的连续多段消息，并支持类似 `tail -f` 的实时流式监听。特别适用于解决“对方打过来好几段话被 summary 截断或覆盖无法及时获知”的问题。

```bash
wechat-cli latest                            # 0.05秒秒提当前活跃会话的最新多段话
wechat-cli latest "张三"                     # 秒出张三发过来的最新多段话
wechat-cli latest "AI交流群" --limit 20      # 查看群内最新 20 条消息
wechat-cli latest --new                      # 增量模式：只看刚新发过来的几句话
wechat-cli latest -f                         # 实时流式跟随监听：对方发消息瞬间在终端刷出！
wechat-cli latest --format json              # 结构化 JSON 导出
```

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `[CHAT_NAME]` | 自动最近活跃 | 聊天对象名称/备注/群名/wxid（可选） |
| `--limit <n>` | `10` | 返回消息条数（默认10条，完整覆盖多段话） |
| `--chats <n>` | `1` | 未指定聊天时，提取最近活跃的会话数 |
| `--new` | 否 | 增量模式：只提取上次调用以来的新增消息 |
| `--all` | 否 | 强制提取最新全部消息（忽略增量状态） |
| `-f, --follow` | 否 | 实时流式跟随监听模式（亚秒级轮询，有新消息立刻打印） |
| `--interval <sec>` | `0.5` | 监听轮询间隔秒数 |
| `--include-official` | 否 | 是否包含公众号与系统服务号（默认只看真人与群聊） |
| `--format text\|json` | `text` | 输出格式（默认优雅可读文本，亦可选 json） |
| `--media` | 否 | 解析媒体文件路径（图片/文件等） |
